...
@pytest.mark.django_db
class TestRevokeStructuredEventView:
    """SC-42: revoke a previously recorded SC-38 structured event."""
    ...
    @responses.activate
    def test_revoke_structured_event_success(self, authenticated_client):
        responses.add(
            responses.POST,
            "https://soroban-testnet.stellar.org/",
            json={"status": "PENDING", "hash": "abc123"},
        ...
        url = reverse("revoke-structured-event")
        data = {"correlation_id": "b" * 64}
        response = authenticated_client.post(url, data, format="json")

        assert response.status_code in [status.HTTP_202_ACCEPTED, status.HTTP_400_BAD_REQUEST]

    def test_revoke_structured_event_validation_error(self, authenticated_client):
        url = reverse("revoke-structured-event")
    ...
    def test_revoke_structured_event_rejects_short_correlation_id(self, authenticated_client):
        url = reverse("revoke-structured-event")
    ...
    def test_revoke_structured_event_requires_authentication(self, api_client):
        url = reverse("revoke-structured-event")
        data = {"correlation_id": "b" * 64}
